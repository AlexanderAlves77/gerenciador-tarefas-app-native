...
export const ModalAdicaoEdicao = (props: any) => {
  ...
  const [dataPrevisaoTarefa, setDataPrevisaoTarefa] =
    React.useState<Date | null>(null)
  const [dataConclusao, setDataConclusao] = React.useState<Date | null>(null)
  const [showData, setShowData] = React.useState(false)
  const [showDataConclusao, setShowDataConclusao] = React.useState(false)
    ...
    >
      <View
        style={[
          ...
          modalVisible ? { backgroundColor: 'rgba(0,0,0,0.5)' } : null,
        ]}
      >
        <View style={modalStyles.container}>
          <Text style={modalStyles.title}>
            {tarefa ? 'Alterar uma tarefa' : 'Adicionar uma tarefa'}
          </Text>
          ...
          >
            <Text style={modalStyles.inputText}>
              {dataPrevisaoTarefa
                ? moment(dataPrevisaoTarefa).format('DD/MM/yyyy')
          ...
          {tarefa !== null && tarefa !== undefined ? (
            <>
              ...
              >
                <Text style={modalStyles.inputText}>
                  {dataConclusao
                    ? moment(dataConclusao).format('DD/MM/yyyy')
                    : 'Data de conclusão'}
                </Text>
              </TouchableOpacity>

              {showDataConclusao === true && (
                <DateTimePicker
                  mode="date"
          ...
          <View style={[defaultStyles.horizontalView, modalStyles.buttons]}>
            <TouchableOpacity
              ...
              style={[
                modalStyles.button,
                tarefa ? modalStyles.buttonsEdit : null,
              ]}
            >
              <Text style={modalStyles.buttonText}>
                {tarefa ? 'Alterar' : 'Salvar'}
              </Text>
            </TouchableOpacity>
            {tarefa ? (
              <TouchableOpacity
